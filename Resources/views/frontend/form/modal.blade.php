...
//dd($ipay);
$apikey 			= $ipay->apikey;
$merchantid 		= $ipay->merchantid;
$accountid 			= $ipay->accountid;
$mode  				= $ipay->mode;
$currency 			= $ipay->currency;
$amount 			= 100;
$referencecode 		= rand();
$responseurl 		= $ipay->replyurl;
$confirmationurl 	= $ipay->confirmationurl;
			...
			<form method="post" action="{{ ($ipay->mode == 1) ? 'https://sandbox.gateway.payulatam.com/ppp-web-gateway/' : 'https://gateway.payulatam.com/ppp-web-gateway/' }}">
				...
				@if($ipay->status == 1)
					<div class="modal-body">
						...
						<input name="test"  type="hidden"  value="{{ $mode }}">
...
<script>
		...
		function reorder(){
			...
			var codigo 			= apikey + '~' + merchantid + '~' + referencecode + '~' + amount + '~' + currency; 
			document.getElementById('signature').value = coding(codigo); 
		}
